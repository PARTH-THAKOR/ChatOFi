...
class ChatBackEnd extends StatefulWidget {
  const ChatBackEnd({Key? key}) : super(key: key);

  @override
  ChatBackEndState createState() => ChatBackEndState();
}

class ChatBackEndState extends State<ChatBackEnd> {
  ...
  massagePage(int index, BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        ...
        child:
            Icon(Icons.send, color: (darkmode) ? Colors.black : Colors.white),
      ),
      appBar: AppBar(
        leading: BackButton(
...
class ChatFrontEnd extends StatefulWidget {
  const ChatFrontEnd({Key? key}) : super(key: key);

  @override
  ChatFrontEndState createState() => ChatFrontEndState();
}

class ChatFrontEndState extends State<ChatFrontEnd> {
  List<Contact>? contacts = [];
  final scroller = ScrollController();
      ...
      body: SafeArea(
        child: Column(
          children: [
            ...
            Container(
              margin: const EdgeInsets.only(
                  left: 15, right: 16, bottom: 16, top: 1),
              child: ListTile(
                tileColor: (darkmode)
                    ? const Color(0xFF424242)
                    : Colors.black.withOpacity(0.2),
      ...
      try {
        ...
        if (number.contains(curruntUser) &&
          ...
          return Container(
            ...
            child: ListTile(
                tileColor: (darkmode) ? const Color(0xFF212121) : Colors.white,
                leading: CircleAvatar(
                  backgroundColor: (darkmode) ? Colors.grey : Colors.pinkAccent,
                  child: (contacts![i].phones.isNotEmpty)
                      ? ContactProfilePhotos(
        ...
        Fluttertoast.showToast(msg: "hello$e");
      }
    }
  }
